...
/** ดึงข้อมูลจริงจาก DB — คืน null ถ้าไม่พบ AN */
async function fetchPatient(an) {
...
export async function GET(req) {
  try {
    ...
    // cache สั้น ๆ 60 วิ — กันกดรัว/หลายคนเปิดเคสเดียวกัน แต่ยังเห็นรหัสที่
    // เพิ่งลงใหม่ได้เร็ว (เคสที่ยังไม่พบก็ cache กันยิงซ้ำเหมือนกัน)
    const data = await cachedQuery(["patient", an], () => fetchPatient(an), 60);
    if (!data) {
      return NextResponse.json(
